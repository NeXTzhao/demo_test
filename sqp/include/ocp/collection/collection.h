...
public:
   ...
   * @return True if the term was in the Collection and false if the term was
   ...
   *
   * @param name: Name of the term.
   * @return A unique pointer to the extracted term. If the term was not found
   * it returns nullptr.
   */
  std::unique_ptr<T> extract(const std::string &name);

  /**
   * Replace a term from the collection.
   * @param name: Name stored along with the term.
   * @param term: Term to be added.
   */
  void replace(std::string name, std::unique_ptr<T> term);

  /**
   * Use to modify a term.
   * @tparam Derived: derived class of base type T to cast to. Casts to the base
   * class by default
   * @param name: Name of the term to modify
   * @return A reference to the underlying term
   */
  template <typename Derived = T> Derived &get(const std::string &name);
  ...
  const std::vector<std::unique_ptr<T>> &get_terms() const;

  const std::unordered_map<std::string, size_t> &get_term_name_map() const;
...
template <typename T> void Collection<T>::clear() {
  terms_.clear();
  termNameMap_.clear();
}

template <typename T> void Collection<T>::reserve(const size_t size) {
  terms_.reserve(size);
  termNameMap_.reserve(size);
}
...
void Collection<T>::add(std::string name, std::unique_ptr<T> term) {
  ...
  if (info.second) {
    ...
    throw std::runtime_error(
        std::string("[Collection::add] Term with name \"") + info.first->first +
        "\" already exists");
  }
}
...
template <typename T>
void Collection<T>::replace(std::string name, std::unique_ptr<T> term) {
  auto it = termNameMap_.find(name);
  if (it == termNameMap_.end()) {
    add(std::move(name), std::move(term));
  } else {
    const size_t index = it->second;
    terms_[index] = std::move(term);
  }
}
...
Collection<T>::Collection(const Collection &other)
    : termNameMap_(other.termNameMap_) {
  // Loop through all terms and clone. The name map can be copied directly
  // because the order stays the same.
...
template <typename T>
const std::vector<std::unique_ptr<T>> &Collection<T>::get_terms() const {
  return terms_;
}

template <typename T>
const std::unordered_map<std::string, size_t> &
Collection<T>::get_term_name_map() const {
  return termNameMap_;
}
